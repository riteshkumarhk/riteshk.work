...
export function VisibilityMenu({ deck, disabled, onChange }) {
  ...
  return <div className="merge-visibility">
    <ToolMenu label={`Slideshow visibility: ${isPublic ? "public" : "owner-only"} draft`} icon={<Icon size={18} strokeWidth={1.75} aria-hidden="true" />} disabled={disabled} showChevron={false}>
      ...
      <p className="merge-visibility-status">Live visibility: not connected</p>
    </ToolMenu>
  </div>;
}

export function VisibilityConfirmation({ onClose, onConfirm }) {
  ...
  return <DeckDialog wide={false} title="Allow public slideshow publishing?" onClose={onClose}>
    <div className="merge-visibility-confirm">
      ...
      <p>This changes the lab draft setting only. Publishing is not connected, and source review will still be required before publication.</p>
    </div>

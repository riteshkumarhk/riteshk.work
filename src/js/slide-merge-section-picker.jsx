...
export function SectionPicker({ title = "Generate from a section", onClose, onPick, embedded = false, multiple = false, mediaOnly = false, onUpload, composition }) {
  ...
  const content = <>
    ...
    {multiple && study && <div className={`merge-section-actions${composition ? " merge-section-actions--ai" : ""}`}><button disabled={loading || !selected.length} onClick={() => onPick(study.blocks.filter((block, index) => selected.includes(index)), resources)}>Generate {selected.length || ""} {selected.length === 1 ? "slide" : "slides"}</button><button disabled={!selected.length} onClick={() => setSelected([])}>Clear</button>{composition && <button disabled={loading || !selected.length} title="Draft slides from selected sections using your configured AI provider" onClick={() => setDrafting(true)}><Sparkles size={15} strokeWidth={1.75} />Draft with AI</button>}</div>}

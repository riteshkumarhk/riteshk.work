...
export function nativeSectionLayers(elements, state) {
  ...
  return visible.filter(element => element.customData?.sectionComponent || element.customData?.sectionReference || element.customData?.slideEmbed || element.customData?.slideDepth).map(element => {

...
export function useMobilePanels(api) {
  ...
  useEffect(() => {
    ...
    return api.onChange((elements, state) => {
      ...
      else setPanel(previous => ["library", "icons", "text", "badges", "sections", "layout", "source", "draft", "media", "layers"].includes(previous) ? null : previous);
    });
  }, [api, mobile]);
  function open(next, selected = false) {
    setPanel(next);
    api?.updateScene({ appState: {
      openSidebar: next === "library" ? { name: "default", tab: "library" } : ["icons", "text", "badges", "sections", "layout", "source", "draft", "media", "layers"].includes(next) ? { name: "insert", tab: next } : null,

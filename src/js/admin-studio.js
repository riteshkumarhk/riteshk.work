...
(function () {
  ...
  async function skimGenerate(i, btn) {
    ...
    var system = "You are a senior product designer distilling your case study into its 'Key Moves' - the 2-3 decisions a hiring UX manager should remember. This is signature portfolio writing: every line must read like something a principal designer would be proud to ship - concise, value-first, quietly confident, never a to-do list, never robotic. LANGUAGE IS THE BAR: lead with the point in the first three words, and NAME the specific thing you built (the coined artifact, framework or pattern) instead of a vague descriptor. Cut every filler word - no stacked adjectives, no sub-clauses, no trailing detail. Each move has three parts with a fixed grammar: problem = a crisp PAST-TENSE tension that leads with the VALUE AT STAKE (trust, focus, adoption, scale, confidence) - what was being lost, not the UI mechanism; move = what YOU DID, led by a strong PAST-TENSE verb (Condensed, Made, Turned, Introduced, Separated, Unified, Replaced, Reframed) and NAMING the specific artifact you built; outcome = the human payoff or the real systems it now serves, plain and value-first. Use ONLY facts from the case; never invent numbers. FORMATTING (a restrained finish - the bronze accent is a SPOTLIGHT, not a highlighter): across the WHOLE set of moves, wrap AT MOST ONE genuinely coined artifact - a proper noun you invented and could trademark, like [[Context Object]] - in [[double brackets]] so it renders in the brand accent. Use it once for the entire case, on the single most distinctive named thing; if no move names a truly coined artifact, use NO accent at all. NEVER accent a verb phrase, a concept or a generic descriptor (not 'authentication the hero', not 'reusable framework', not 'design system'). Separately you may bold ONE pivotal word with **double asterisks** in a single outcome line. Never format the problem line. Default to restraint: one bronze word in the whole set, sometimes zero. Order moves strongest-first so the three read as one narrative. Return STRICT JSON only, no prose.";
    var user = "CASE STUDY:\n" + g.text + visualNote +
      "\n\nWrite the 2-3 Key Moves for THIS case. Match the LANGUAGE QUALITY, brevity, rhythm and light formatting of this gold-standard example EXACTLY - copy the STYLE, never the words:" +
      "\n{\"beats\":[" +
      ...
      "\n]}" +
      "\n\nRules: LANGUAGE IS THE BAR - each line concise and value-first, leading with the point in its first three words. problem = a complete PAST-TENSE tension that names the VALUE AT STAKE (e.g. 'Trust was buried...', NOT 'Context was buried inside dense, unreadable dialog text'); cut filler adjectives and sub-clauses. move = STARTS WITH A PAST-TENSE VERB (Condensed/Made/Turned/Introduced/Separated/Unified/Replaced/Reframed) and NAMES the specific artifact you built - a coined noun like 'Context Object' or 'Credential UX framework', never a vague descriptor like 'glanceable object' or 'reusable thing'; never an imperative (not 'Demote', 'Separate', 'Condense'). outcome = the human payoff in plain confident words, naming the real systems or scenarios where it now lives. Write moves true to THIS case - never reuse the example's words. FORMATTING (restraint is the whole point - the accent is a spotlight, not a highlighter): across ALL the moves together, put [[double brackets]] around AT MOST ONE genuinely coined artifact - a proper noun you could trademark, like [[Context Object]] above. Notice the example accents ONLY that first move; moves 2 and 3 carry NO accent, because 'authentication the hero' and 'reusable framework' are plain concepts, not coined names. Do the same: one bronze word for the whole case, zero if nothing is truly coined, NEVER on a verb phrase or a generic descriptor. You may bold ONE pivotal word with **...** in a single outcome line. Never format the problem line. No cryptic fragments ('framework born'), no forced verbs ('crown'), no vague abstractions ('all Windows auth'). LENGTH IS CRITICAL - match the brevity of the examples exactly: problem <= 9 words, move <= 10 words, outcome <= 9 words, each ONE line that never wraps. The examples are the MAXIMUM; if a line reads like a full sentence with sub-clauses or trailing detail, cut it to its essence. Return STRICT JSON {\"beats\":[{\"problem\":\"\",\"move\":\"\",\"outcome\":\"\"}]} only.";
    try {

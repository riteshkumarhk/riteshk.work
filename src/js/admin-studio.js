...
(function () {
  ...
  async function skimGenerate(i, btn) {
    ...
    var user = "CASE STUDY:\n" + g.text + visualNote +
      ...
      "\n]}" +
      "\n\nRules: problem = a complete PAST-TENSE tension (e.g. 'Trust was buried...', 'Every scenario was...'). move = STARTS WITH A PAST-TENSE VERB (Condensed/Made/Turned/Introduced/Separated/Unified/Replaced/Reframed) naming the specific thing you built - NEVER an imperative command (not 'Demote', 'Separate', 'Condense'). outcome = the human payoff in plain confident words, naming the real systems or scenarios where they exist. Write moves true to THIS case - do not reuse the example's words. No cryptic fragments ('framework born'), no forced verbs ('crown'), no vague abstractions ('all Windows auth'). LENGTH IS CRITICAL - match the brevity of the examples exactly: problem <= 9 words, move <= 10 words, outcome <= 9 words, each ONE line that never wraps. The examples are the MAXIMUM length; if a line reads like a full sentence with sub-clauses or trailing detail (e.g. 'surfaced at the top of every auth dialog', 'forcing users to read and interpret'), cut it to its essence. Return STRICT JSON {\"beats\":[{\"problem\":\"\",\"move\":\"\",\"outcome\":\"\"}]} only.";
    try {

...
(function () {
  ...
  function stageHtml(w, st, hasBlocks) {
    var slides = stageSlides(w, st);
    if (slides.length <= 1) {
      var hasCover = !!(w.image || (st && st.cover));
      return (hasCover || hasBlocks) ? coverHtml(w, st) : "";
    }
    ...
    return '<section class="pj__stage" data-stage data-count="' + slides.length + '">' +
      '<div class="pj__stage-main" data-stage-main>' + mains +
        '<div class="pj__stage-bar" aria-hidden="true"><span class="pj__stage-bar-fill" data-stage-fill></span></div>' +
        '<button class="pj__stage-fs" type="button" data-stage-fs aria-label="View fullscreen" title="Fullscreen">' + FS_SVG + "</button>" +
      "</div>" +
      '<div class="pj__stage-strip" data-stage-strip>' + thumbs + "</div>" +
    "</section>";
  }
  ...
  function pvToolbarHtml(idx, total, locked, flush) {
    ...
    return '<button type="button" class="pjtb__b" data-pjtb="add" title="Add a section above" aria-label="Add a section above">' + ic('<path d="M5 12h14"/><path d="M12 5v14"/>') + '</button>' +
      ...
      '<button type="button" class="pjtb__b pjtb__b--lock' + (locked ? " is-locked" : "") + '" data-pjtb="lock" title="' + (locked ? "Locked \u2014 click to unlock" : "Lock \u2014 deeper-cut only") + '" aria-label="Toggle lock">' + ic(locked ? '<rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>' : '<rect x="3" y="11" width="14" height="11" rx="2"/><path d="M13 11V6a4 4 0 0 1 8 0v2"/>') + '</button>';
  }
  ...
  function onOverlayClick(e) {
    ...
    if (zoomImg || richImg) {
      ...
      if (zoomImg) {
        groupRoot = zoomImg.closest(".pjb__gallery, .pjb__media, .pjb__mediagrid, .pjb__devices, .pj__stage-main");
    ...
    if (dotsBtn) { var sheetOpen = overlay.classList.toggle("pj--sheet-open"); dotsBtn.setAttribute("aria-expanded", sheetOpen ? "true" : "false"); if (window.__rklog) window.__rklog("sys", "section menu " + (sheetOpen ? "opened" : "closed")); return; }
  ...
  function initStage(scope) {
    ...
    if (count < 2) return;
    var main = stage.querySelector("[data-stage-main]");
    var strip = stage.querySelector("[data-stage-strip]");
    ...
    function setFill(p) { if (fill) fill.style.width = Math.max(0, Math.min(100, p)) + "%"; }
    function resetBar(transition) { if (!fill) return; fill.style.transition = "none"; fill.style.width = "0%"; void fill.offsetWidth; fill.style.transition = transition || "none"; }
    ...
    function show(i) {
      ...
      resetBar("none");
      if (curVid && fill) fill.style.transition = "width .3s linear";
      playCurrent();
    }
  ...
  function applyNav(w) {
    ...
    overlay.classList.remove("pj--sheet-open", "pj--atfull", "pj--scrolled");
  ...
  function sdbgOn() { return /[?&]sdbg/.test(location.search); }
  function sdbgPlace() {
    if (!sdbgOn()) return;
  ...
  function scrollToAnchor(anchor, extra) {
    ...
    var repin = function () {
      if (dead) return;
      var t = aim(); if (Math.abs(t - scroller.scrollTop) > 2) { if (window.__rklog) window.__rklog("sys", "re-anchored to " + t + "px (content grew above the target)"); scroller.scrollTop = t; }
    };
    ...
    if (window.__rklog) window.__rklog("sys", "scroll jump: " + Math.round(from) + "px \u2192 target " + aim0 + "px");
    var reduce = (window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches) || !window.requestAnimationFrame;
    if (reduce) { scroller.scrollTop = aim(); pin(); sdbg("id=" + (anchor.id || "?") + " reduce=1 from=" + Math.round(from) + " aim=" + aim0 + " final=" + Math.round(scroller.scrollTop)); if (window.__rklog) window.__rklog("sys", "landed at " + Math.round(scroller.scrollTop) + "px (instant, reduced-motion)"); return; }
    ...
    var step = function (ts) {
      ...
      else { raf = 0; scroller.scrollTop = aim(); pin(); sdbg("id=" + (anchor.id || "?") + " reduce=0 from=" + Math.round(from) + " aim0=" + aim0 + " aimE=" + aim() + " final=" + Math.round(scroller.scrollTop) + " f=" + frames); if (window.__rklog) window.__rklog("sys", "landed at " + Math.round(scroller.scrollTop) + "px (" + frames + "-frame glide)"); }
    };
  ...
  function updateSpy() {
    ...
    // The orientation pill is noise while pinned to the very top - reveal it the moment the page scrolls
    // off the top (it reads "Overview" until a section slides under the chrome, then the section name).
    overlay.classList.toggle("pj--scrolled", scroller.scrollTop > 40);
    ...
    if (id !== lastSpyId) {
      lastSpyId = id;
      if (window.__rklog) window.__rklog("sys", "now viewing: " + (id || "\u2014"));
  ...
  function closeSheet() { if (!overlay) return; var wasOpen = overlay.classList.contains("pj--sheet-open"); overlay.classList.remove("pj--sheet-open"); var d = overlay.querySelector("[data-dots]"); if (d) d.setAttribute("aria-expanded", "false"); if (wasOpen && window.__rklog) window.__rklog("sys", "section menu closed"); }
  function gotoSection(id) {
    if (id === "__intro") {
      cancelPin();
      if (window.__rklog) window.__rklog("nav", "jump to overview (top)");
    ...
    if (window.__rklog) window.__rklog("nav", "jump to section " + id + " (" + matches.length + " match" + (sec ? "" : ", NOT FOUND") + ")");

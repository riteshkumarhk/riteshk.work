...
export function installPresenterPanel({ doc, onCommand, onEdit, onJump, renderThumbnail, onResize = () => {}, storage }) {
  ...
  const preserveSelection = event => { if (event.target.closest('.pp__notesformat')) event.preventDefault(); };
  ...
  return {
    ...
    dispose() { observer.disconnect(); rich?.dispose(); doc.removeEventListener('pointerdown', preserveSelection); doc.removeEventListener('click', click); }
  };
}
...
import { createDeckHistory, deckDocumentKey } from "./slide-merge-history.mjs";
import { createStudioDeck } from "./slide-studio-deck.mjs";
...
import { Embed as NativeEmbed, NativeSections, SectionThumbnail, SectionVisibilityMenu, CanvasVideo, Presenter, engineOptions, sceneBackground, changed, validEmbed } from "./slide-studio-renderer.jsx";
...
function Merger({ integration, controller }) {
  ...
  const [status, setStatus] = useState(integration ? "Loading case-study slides" : "Loading local draft"), [selection, setSelection] = useState("[]"), [hasSelection, setHasSelection] = useState(false);
  useEffect(() => { integration?.onStatus?.(status); }, [integration, status]);
  ...
  const [coverSourceError, setCoverSourceError] = useState("");
  useEffect(() => {
  ...
  const activity = useActivity(api, live, integration?.caseStudyId);
  ...
  async function save(recordHistory = true) {
    if (!controller.active) throw new Error("The slide editor session is closed");
  ...
  function enqueueSave(snapshot) {
    const key = integration ? JSON.stringify([deckDocumentKey(snapshot), snapshot.selected]) : null;
    live.current.pendingSaves = (live.current.pendingSaves || 0) + 1;
    live.current.queue = live.current.queue.catch(() => {}).then(async () => {
      if (!controller.active) throw new Error("The slide editor session is closed");
      if (integration && key === live.current.persistedKey) return;
      await (integration ? integration.save(snapshot) : deckStore(snapshot));
      live.current.persistedKey = key;
    }).finally(() => { live.current.pendingSaves--; });
  }
  async function flushEditor() {
    await live.current.operation?.catch(() => {});
    if (!controller.active) throw new Error("The slide editor session is closed");
    if (live.current.gesture != null || host.current?.closest('.merge-shell')?.querySelector('[data-scrubbing]') || api?.getAppState().isDragging || api?.getAppState().isResizing) throw new Error("Finish the current gesture before leaving the slide editor");
    const focused = document.activeElement;
    if (host.current?.closest('.merge-shell')?.contains(focused)) focused.blur();
  ...
  async function run(operation) {
    if (live.current.operating || !controller.active) return;
    live.current.operating = true; setBusy(true);
    const pending = (async () => operation())();
    live.current.operation = pending;
    try { return await pending; } catch (error) { fail(error); } finally { live.current.operating = false; if (controller.active) setBusy(false); }
  }
  ...
  async function resolveCoverSource(previous, files, allowMissingMedia = false) {
    ...
    if (source.depth?.on !== false && !source.depth?.noMap && source.depth?.map && merged.image && !merged.source.overrides.includes("image")) {
      if (previous.depth?.source === source.depth.map && nextFiles[previous.depth.fileId]) merged.depth = { ...source.depth, fileId: previous.depth.fileId, source: source.depth.map };
      else try {
        ...
        if (url) {
          ...
          if (response.ok) {
            ...
            if (/^image\//.test(blob.type)) {
              const image = await originalImage(blob); nextFiles[image.id] = image;
              merged.depth = { ...source.depth, fileId: image.id, source: source.depth.map };
            }
          }
        }
      } catch {}
  ...
  function replaceCoverElements(elements, cover) {
    ...
    const ids = new Set(additions.map(element => element.id));
    return [...additions, ...elements.filter(element => element.customData?.slideCover && !ids.has(element.id)).map(element => element.isDeleted ? element : changed(element, { isDeleted: true })), ...elements.filter(element => !element.customData?.slideCover)];
  }
  function refreshCoverSource(reset) { return run(async () => {
    try {
      const elements = api.getSceneElementsIncludingDeleted(), previous = slideSettings(elements).cover;
      if (!previous) return;
      const source = previous.source ? { ...previous.source, overrides: previous.source.overrides.filter(key => key !== reset) } : undefined;
      const resolved = await resolveCoverSource({ ...previous, source }, api.getFiles());
      commitSettings({ cover: resolved.cover }, replaceCoverElements(elements, resolved.cover));
      api.addFiles(Object.values(resolved.files)); await save(); setCoverSourceError("");
    } catch (error) { setCoverSourceError(error.message); }
  }); }
  function updateCover(update) {
    ...
    try {
      ...
      const source = previous.source ? { ...previous.source, overrides: [...new Set([...previous.source.overrides, ...COVER_SOURCE_FIELDS.filter(key => Object.hasOwn(update, key))])] } : undefined;
      const cover = coverValues({ ...previous, ...update, source });
      commitSettings({ cover }, replaceCoverElements(elements, cover));
      return "";
  ...
  function importMedia(source, purpose = "insert") {
    ...
    return run(async () => {
      ...
      if (purpose === "cover-image" || purpose === "cover-logo") {
        ...
        const key = purpose === "cover-logo" ? "logo" : "image";
        const cover = coverValues({ ...previous, ...(key === "logo" ? { mark: "" } : {}), ...(previous.source ? { source: { ...previous.source, overrides: [...new Set([...previous.source.overrides, key])] } } : {}), [key]: { fileId: image.id, width: image.width, height: image.height, name: source.name || source.title || "Cover image" } });
  ...
  useEffect(() => {
    ...
    let active = true;
    run(async () => {
      const stored = integration ? await integration.load() : await deckStore();
      ...
      const next = stored ? structuredClone(stored) : integration ? createStudioDeck(integration.title) : createDeck(); next.slides = await Promise.all(next.slides.map(materialize));
      let refreshed = false;
      if (integration?.coverSource) for (const slide of next.slides) {
        const previous = slideSettings(slide.scene.elements).cover;
      ...
      paint(next); await mountSlide(next.slides.find(slide => slide.id === next.selected));
      if (integration) {
        capture();
        live.current.persistedKey = refreshed ? null : JSON.stringify([deckDocumentKey(next), next.selected]);
        deckHistory.current.reset(next); live.current.savedRevision = live.current.revision;
        if (refreshed) await save(false);
        setStatus(integration?.savedStatus?.() || "Saved on this device");
      } else await save();
      next.slides.forEach(slide => thumbnail(slide).catch(() => {}));
      controller.resolve();
    }).catch(controller.reject);
    const flush = () => { if (document.visibilityState === "hidden") save().catch(fail); };
    const leave = event => { if (live.current.revision !== live.current.savedRevision || live.current.pendingSaves || live.current.gesture != null || live.current.ready && live.current.operating || api.getAppState().editingTextElement) { event.preventDefault(); event.returnValue = ""; } };
    ...
    const debug = { api, save, choose, importImage, deck: () => structuredClone(live.current.deck) };
    if (!integration) window.__slideMerge = debug;
    return () => { active = false; observer.disconnect(); clearTimeout(live.current.timer); document.removeEventListener("visibilitychange", flush); window.removeEventListener("beforeunload", leave); if (window.__slideMerge === debug) delete window.__slideMerge; };
  }, [api]);
  ...
  const editorBar = <EditorBar historyRef={setHistoryTarget} caseStudyId={integration?.caseStudyId} busy={busy || present !== null || !!deckDialog} editing={editing} onEditing={switchEditing} slideView={slideView} onView={switchView} onPlay={integration?.openPreview || rehearse} canPlay={!!rehearsal.length} newTab={!!integration?.openPreview} />;
  const visibilityControl = <VisibilityMenu deck={deck} publication={integration ? integration.publication?.() || {} : null} disabled={busy || present !== null || !!deckDialog} onChange={isPublic => { if (isPublic) { openPane(null, false); setDeckDialog({kind:"visibility"}); } else changeVisibility(false); }} />;
  const editorStatus = integration?.statusbar ? visibilityControl : <footer className="merge-status" aria-label="Document status"><StatusControls status={status === "Saved on this device" && activity.message ? `${activity.message} - saved` : status} activity={activity}>{visibilityControl}</StatusControls><span className="merge-slide-position">{selectedIndex + 1} / {deck?.slides.length || 0}</span></footer>;
...
export function mountSlideEditor(container, integration = null) {
  const root = createRoot(container);
  const controller = { active: true };
  const ready = new Promise((resolve, reject) => { controller.resolve = resolve; controller.reject = reject; });
  ready.catch(() => {});
  root.render(<Merger integration={integration} controller={controller} />);
  return {
    ready,

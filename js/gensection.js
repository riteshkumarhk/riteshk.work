(() => {
  ...
  // src/js/gensection.js
  var require_gensection = __commonJS({
    "src/js/gensection.js"(exports, module) {
      (function(root) {
        "use strict";
        ...
        var FX = { orbit: 1, tilt: 1, spin: 1 };
        function pick(v, table, dflt) {
          return v != null && table[v] ? v : dflt;
        }
        function pickNum(v, table, dflt) {
          return v != null && table[String(v)] ? Number(v) : dflt;
        }
        function esc(s) {
        ...
        function cleanLeaf(t, n) {
          switch (t) {
            case "heading":
              return { type: t, text: String(n.text || ""), size: pick(n.size, SIZES, "lg") };
            case "text":
              return { type: t, text: String(n.text || ""), size: pick(n.size, SIZES, "md"), tone: pick(n.tone, TONES, "default"), align: pick(n.align, ALIGNS, "left") };
            case "quote":
              return { type: t, text: String(n.text || ""), cite: String(n.cite || "") };
            case "stat":
              return { type: t, value: String(n.value || ""), label: String(n.label || "") };
            case "pill":
              return { type: t, text: String(n.text || ""), tone: pick(n.tone, TONES, "accent") };
            case "icon":
              return { type: t, name: String(n.name || "").replace(/[^a-z0-9_-]/gi, "").slice(0, 40), size: pick(n.size, SIZES, "md") };
            case "media":
              return { type: t, src: safeUrl(n.src), kind: pick(n.kind, MEDIA_KINDS, "image"), ratio: pick(n.ratio, RATIOS, "16x9"), fit: pick(n.fit, FITS, "cover"), alt: String(n.alt || "").slice(0, 200), caption: String(n.caption || "").slice(0, 300) };
            case "button":
              return { type: t, label: String(n.label || "").slice(0, 120), href: safeUrl(n.href) };
            case "divider":
              return { type: t };
            case "spacer":
              return { type: t, size: pick(n.size, SIZES, "md") };
          }
          return null;
        }
        function cleanNode(n, depth) {
          ...
          if (LEAVES[t]) {
            var lf = cleanLeaf(t, n);
            if (lf) {
              if (n.style) lf.style = sanitizeStyle(n.style);
              if (n.fx && FX[n.fx]) lf.fx = n.fx;
            }
            return lf;
          }
          return null;
        ...
        function contProps(p) {
          return " gs-gap-" + p.gap + " gs-al-" + p.align + " gs-va-" + p.valign + (p.pad !== "none" ? " gs-pad-" + p.pad : "") + (p.radius !== "none" ? " gs-rad-" + p.radius : "") + (p.bg !== "none" ? " gs-bg-" + p.bg : "");
        }
        ...
        function styleAttr(n) {
          return n.style ? ' style="' + esc(n.style) + '"' : "";
        }
        function fxAttr(n) {
          return n.fx ? ' data-rk-fx="' + esc(n.fx) + '"' : "";
        }
        function renderNode(n) {
          ...
          switch (t) {
            case "heading":
              return '<h3 class="gs-h gs-h--' + n.size + '"' + sa + ">" + rich(n.text) + "</h3>";
            case "text":
              return '<div class="gs-text gs-tx--' + n.size + " gs-tone-" + n.tone + " gs-al-" + n.align + '"' + sa + ">" + rich(n.text) + "</div>";
            case "quote":
              return '<blockquote class="gs-quote"' + sa + ">" + rich(n.text) + (n.cite ? '<cite class="gs-cite">' + esc(n.cite) + "</cite>" : "") + "</blockquote>";
            case "stat":
              return '<div class="gs-stat"' + sa + '><span class="gs-stat-v">' + esc(n.value) + '</span><span class="gs-stat-l">' + esc(n.label) + "</span></div>";
            case "pill":
              return '<span class="gs-pill gs-pill--' + n.tone + '"' + sa + ">" + esc(n.text) + "</span>";
            case "icon":
              return '<span class="gs-icon gs-icon--' + n.size + '"' + sa + ">" + (iconSvg(n.name) || '<span class="gs-icon-dot"></span>') + "</span>";
            case "media":
              return '<figure class="gs-media gs-media--' + n.ratio + " gs-fit-" + n.fit + '"' + sa + ">" + mediaInner(n) + (n.caption ? '<figcaption class="gs-cap">' + esc(n.caption) + "</figcaption>" : "") + "</figure>";
            case "button":
              return n.href ? '<a class="gs-btn" href="' + esc(n.href) + '" target="_blank" rel="noopener"' + sa + ">" + esc(n.label) + "</a>" : '<span class="gs-btn gs-btn--dead"' + sa + ">" + esc(n.label) + "</span>";
            case "divider":
              return '<hr class="gs-divider"' + sa + ">";
            case "spacer":
              return '<div class="gs-spacer gs-sp--' + n.size + '"' + sa + "></div>";
          }
          return "";
        }
        ...
        function attachFx(el) {
          ...
          function apply() {
            el.style.transform = "perspective(" + persp + ") rotateX(" + rx + "deg) rotateY(" + ry + "deg) scale(" + scale + ")";
          }
          apply();
          if (mode === "tilt") {
          ...
          el.addEventListener("wheel", function(e) {
            e.preventDefault();
            scale = Math.max(0.4, Math.min(2.6, scale * (e.deltaY > 0 ? 0.92 : 1.08)));
            apply();
          }, { passive: false });
          ...
          if (mode === "spin" && !reduce) {
            ...
            requestAnimationFrame(function loop(ts) {
              ...
              if (last && !dragging && !hover) {
                ry = (ry + (ts - last) * 0.02) % 360;
                apply();
              }
              last = ts;
        ...
        root.RKGen = {
          ...
          vocab: {
            ...
            aligns: Object.keys(ALIGNS),
            valigns: Object.keys(VALIGNS),
            gaps: Object.keys(GAPS),
            pads: Object.keys(PADS),
            cols: Object.keys(COLS),
            radii: Object.keys(RADII),
            bgs: Object.keys(BGS),
            ratios: Object.keys(RATIOS),
            fits: Object.keys(FITS),
            mediaKinds: Object.keys(MEDIA_KINDS)
          }
        };

...
(function () {
  ...
  function blankBlock(type) {
    switch (type) {
      ...
      case "workflow": return { type: "workflow", nav: "", kicker: "", heading: "", flow: "linear", src: "", caption: "", items: [] };
  ...
  function blockEditor(i, b, j, len, open) {
    ...
    else if (b.type === "workflow") { var wfCycle = b.flow === "cycle"; body = sfInput(i, j, "heading", "Heading") + sfSelect(i, j, "flow", "Layout", [["linear", "Linear \u2014 left to right"], ["loop", "Loop \u2014 a repeating cycle"], ["cycle", "Cycle \u2014 a radial loop with a centre"]], "How the steps connect.") + (wfCycle ? mediaInputBlock(i, j, "src", "Centre image (optional)") : "") + itemRepeater(i, j, b) + sfInput(i, j, "caption", "Caption") + '<div class="af__hint">' + (wfCycle ? "Cycle spaces your steps evenly around a ring with curved arrows and an optional centre image \u2014 best with 3\u20136 short steps. On phones it stacks into a clean list." : "Steps flow left to right with arrows. Split a step with <b>//</b> to fork into parallel branches that merge back \u2014 e.g. <em>Design // Eng // Legal</em>.") + "</div>"; }
  ...
  function onStudyBlock(t) {
    ...
    if (b.type === "workflow" && f === "flow") { saveDraft(); renderL2(); return; }
    saveDraft();
    refreshL2Preview();
  }

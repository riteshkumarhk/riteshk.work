...
export function cornerEnginePlugin() {
  ...
  return { name: "lab-corner-geometry", setup(build) {
    ...
    build.onLoad({ filter: /excalidraw[\\/]dist[\\/]dev[\\/]index\.js$/ }, async ({ path }) => {
      ...
      const edits = [
        ['var StaticCanvas = (props) => {', 'var StaticCanvas = (props) => {\n  const [labAppearance, setLabAppearance] = React42.useState(() => document.documentElement.dataset.appearance || "dark");\n  useEffect42(() => {\n    const update = () => setLabAppearance(document.documentElement.dataset.appearance || "dark");\n    window.addEventListener("theme:change", update);\n    return () => window.removeEventListener("theme:change", update);\n  }, []);\n  const labVisibleElements = React42.useMemo(() => labCoverAppearanceElements(props.visibleElements, labAppearance), [props.visibleElements, labAppearance]);'],
        ['        visibleElements: props.visibleElements,\n        appState: props.appState,\n        renderConfig: props.renderConfig', '        visibleElements: labVisibleElements,\n        appState: props.appState,\n        renderConfig: props.renderConfig'],
        ['        isMobile: this.isMobileBreakpoint(editorWidth, editorHeight),', '        isMobile: container.closest(".merge-workspace") && window.matchMedia("(min-width:901px)").matches ? false : this.isMobileBreakpoint(editorWidth, editorHeight),'],
        ['var EyeDropper = ({ onCancel, onChange, onSelect, colorPickerType }) => {', 'var EyeDropper = ({ onCancel, onChange, onSelect, colorPickerType, canvasOnly }) => {\n  const screenPicker = !canvasOnly && typeof window.EyeDropper === "function";'],
        ['  useEffect9(() => {\n    const colorPreviewDiv = ref.current;', '  useEffect9(() => {\n    if (!screenPicker) return;\n    return labOpenScreenEyeDropper({\n      onSelect: color => {\n        const visible = colorPickerType === "canvasBackground" || !stableProps.selectedElements.length && !!excalidrawContainer?.querySelector(".merge-slide-color");\n        excalidrawContainer?.focus();\n        stableProps.onSelect(labScreenColorForCanvas(color, app.canvas, visible), { altKey: false });\n      },\n      onCancel: () => { excalidrawContainer?.focus(); stableProps.onCancel(); },\n      onError: () => app.setToast({ message: "Screen colour picking is unavailable. Try again or use a colour value.", closable: true })\n    }, window);\n  }, [screenPicker, stableProps, app, colorPickerType, excalidrawContainer]);\n  useEffect9(() => {\n    if (screenPicker) return;\n    const colorPreviewDiv = ref.current;'],
        ['    stableProps,\n    app.canvas,\n    eyeDropperContainer,', '    stableProps,\n    screenPicker,\n    app.canvas,\n    eyeDropperContainer,'],
        ['  if (!eyeDropperContainer) {\n    return null;\n  }', '  if (screenPicker || !eyeDropperContainer) {\n    return null;\n  }'],
        ['        colorPickerType: eyeDropperState.colorPickerType,', '        colorPickerType: eyeDropperState.colorPickerType,\n        canvasOnly: eyeDropperState.keepOpenOnAlt,'],
        ['          title: `${t(\n            "labels.eyeDropper"\n          )}', '          title: `${typeof window.EyeDropper === "function" ? "Pick colour from screen" : t("labels.eyeDropper")}'],
        ['      options.push(copyText);', '      options.push(copyText);\n      if (document.querySelector(".merge-shell") && !this.state.viewModeEnabled) {\n        const selected = this.scene.getNonDeletedElements().filter(element => this.state.selectedElementIds[element.id]);\n        if (selected.length === 1 && (selected[0].customData?.sectionComponent || selected[0].customData?.sectionReference)) options.push({ name: "labSectionVisibility", trackEvent: false, perform: (_elements, state) => { document.dispatchEvent(new CustomEvent("rk:section-visibility", { detail: { id: selected[0].id, left: state.contextMenu?.left, top: state.contextMenu?.top } })); return false; } });\n      }\n      if (document.querySelector(".merge-shell") && !this.state.viewModeEnabled && this.scene.getNonDeletedElements().some(element => element.type === "text" && (this.state.selectedElementIds[element.id] || this.state.selectedElementIds[element.containerId]))) options.push({ name: "labImproveText", trackEvent: false, perform: () => { document.dispatchEvent(new CustomEvent("rk:improve-slide-text")); return false; } });'],
        ['              let label = "";\n              if (item.label) {', '              let label = actionName === "labImproveText" ? "Improve with AI" : actionName === "labSectionVisibility" ? "Show/hide" : "";\n              if (item.label) {'],
      ...
      source = `import { coverAppearanceElements as labCoverAppearanceElements } from ${JSON.stringify(resolve("src/js/slide-merge-appearance.mjs").replaceAll("\\", "/"))};\n` + source;

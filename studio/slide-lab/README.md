...
New covers snapshot the active site's background, raised surfaces and text colour tokens, alongside its font roles. Standard cover colours follow light/dark appearance in the editor, slideshow and previews through a display-only projection; custom colours and original images remain unchanged. Switching appearance never rewrites the saved palette. **Use site colours** explicitly replaces saved colours through normal undo/save. Previously published fixed covers with stripped editor metadata are recognized by their original background, rail and media-panel geometry; arbitrary custom-background slides are not converted. Team entries separated by commas or newlines become individual chips. **Upload brand logo**, beside Client, replaces the initials input and supports original-image upload, replacement and removal without stretching.

The slide editor's Add menu has an explicit **Add cover** action. It creates a fixed 1280x720 composition with a brand rail, title, status/duration, up to eight team entries, role, footnote and hero image. The existing left inspector owns its content, original-image/logo selection, colours and transition; mobile uses the existing properties sheet. Empty optional fields are omitted, long copy fits its fixed bounds, the hero is cropped non-destructively and a logo is contained. Cover objects start locked and remain ordinary native slide elements, so thumbnails, presentation, saved layouts and audience rendering need no new renderer. Existing slides are not converted. Automated checks cover field updates, actual canvas image pixels, original image bytes, duplication, undo/redo, reload, desktop/mobile bounds and the public audience payload with editor-only cover fields stripped. The reference's actual image is user-supplied, not bundled into the template.

On desktop Edge/Chrome over HTTPS or localhost, the pipette (or I shortcut) uses the browser's native screen eyedropper. It samples visible pixels from inserted sections, images, videos, embedded pages and the surrounding screen, then applies one colour through the existing stroke, fill, text or slide-background callback. Escape cancels; leaving the picker aborts pending results. No screenshot is uploaded or saved by this feature. Unsupported browsers and the Alt-held shortcut retain the limited canvas/video sampler; cross-origin embeds and surrounding UI require native screen support. The native OS selection overlay is not a DOM control: automated tests supply screenshot-derived section/header pixels at its API boundary, while the shared browser verifies real native open/cancel. Full GPU-PC pixel selection remains a manual check.

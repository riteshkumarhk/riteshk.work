export class PasskeyChallenges {
  ...
  async fetch(request) {
    ...
    if (action === "/issue") {
      ...
      const record = { type: input.type, purpose: ["publish", "release-checks", "security"].includes(input.purpose) ? input.purpose : "login", exp: Date.now() + 300000, remember: input.remember === true, session: input.session || "" };
